...
/*
  ...
  - m/d/[yy[yy]]: examples: '7/21', '7/21/07', '7/21/2007' *handy_patriawan - Added

*/
//*handy_patriawan - Add 'format' parameter
function string_to_date(s, format) {
  var result = undefined;
  var today = new Date;
  var indexDate = undefined;
  var indexMonth = undefined;
  ...
  if (mo = s.match(/^\s*(\d{4})-(\d{1,2})-(\d{1,2})\s*$/)) {
	...
	//*handy_patriawan - Handles month/date/year format
	if (format == "%m/%d/%Y") {
	  indexDate = 2;
	  indexMonth = 1;
	}
    ...
    //*handy_patriawan - Use index* variable instead of hard-coding month to 2 and date to 1
    mo[indexMonth] = mo[indexMonth] ? Number(mo[indexMonth])-1 : today.getMonth();
    mo[3] = mo[3] || today.getFullYear();
    mo[3] = Number(mo[3]);
    if (mo[3] < 99) mo[3] += 2000;
    result = new Date(mo[3], mo[indexMonth], mo[indexDate]);
  }
  return result;
}
...
function calendar_open(input_element, options) {
  ...
  calendar_show();
  // Parse input date.
  var date = string_to_date(input_element.value, calendar.format); //*handy_patriawan - Add calendar.format
  if (date) {
