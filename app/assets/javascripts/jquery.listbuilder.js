...
//add body role
	...
	var options = $.extend({
			delimChar: ",", //character used to split textarea content into items
	...
	var listbuilder = $('<ul class="listbuilder ui-corner-all"></ul>')
							.width(options.width);
	
	//function to return a new listbuilder entry
	function listUnit( val ){ 
		return $('<li class="listbuilder-entry"><span class="listbuilder-entry-text ui-corner-all">'+val+'</span><a href="#" class="listbuilder-entry-remove" title="Remove '+val+' from the list." role="button"></a></li>')
			.hover(
